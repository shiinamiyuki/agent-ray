...
use agent_ray::integrators::{Integrator, Pssmlt, PssmltConfig};
...
fn main() {
    ...
    let config = PssmltConfig {
        ...
        large_step_prob: 0.1,
        sigma: 0.01,
        image_mutation_size: 0.1,
    };
    let integrator = Pssmlt::new(config);

    println!(
        "Rendering {}×{} @ ~{}spp (PSSMLT, {} chains)…",
        width, height, integrator.config.spp, integrator.config.n_chains,
    );
    ...
    // Save output
    // -----------------------------------------------------------------------
    let out = "pssmlt_test.png";
    film.to_rgb_image(ToneMapper::Reinhard, 2.2, 1.0)
        .save(out)
        .expect("Failed to save image");
    println!("Saved → {out}");

    let exr_out = "pssmlt_test.exr";
